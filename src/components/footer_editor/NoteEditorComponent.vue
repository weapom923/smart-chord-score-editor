<template>
  <v-card v-bind="$attrs" v-if="$_selectedNote">
    <v-card-text class="pa-0">
      ...
      />
      <v-card-subtitle>Note Type</v-card-subtitle>
      <note-type-selector-buttons
        v-bind:note-type="$_selectedNoteType"
        v-on:change="$_onChangeNoteType"
      />
      <template v-if="$_isChordNoteAvailable">
        <v-card-subtitle>Chord</v-card-subtitle>
        <chord-editor-component
          v-model="$_selectedNote.pitchOrChord"
        />
      </template>
    </v-card-text>
  </v-card>
...
export default {
  ...
  computed: {
    ...
    $_isChordNoteAvailable() {
      ...
      if (this.$_selectedNote.pitchOrChord === null) return false;
      return true;
    },
  ...
  methods: {
        ...
        case Note.Type.normal:
          switch (this.$_selectedPart.type) {
            case PartInBar.Type.chord:
              notes[this.selectedNoteIdx] = new Note(this.$store.state.config.defaultChord, this.$_selectedNote.value, noteType);
              break;
          }
          break;
        case Note.Type.rest:
          notes[this.selectedNoteIdx] = new Note(null, this.$_selectedNote.value, noteType);
          break;
      }
    ...
    $_onChangeValues() {
      ...
      let newNoteIdx = this.selectedNoteIdx;
    ...
    $_onInputValues([ noteValueSliderValueLow, noteValueSliderValueHigh ]) {
      ...
      if (insertRestNoteToFront) {
        ...
        newNotes.push(new Note(null, noteValue, Note.Type.rest));
      }
      for (let currentNoteIdx = 0; currentNoteIdx < numNotesInPart; ++currentNoteIdx) {
        let currentNote = this.$_selectedPart.notes[currentNoteIdx];
        if (currentNoteIdx === this.selectedNoteIdx) {
          newNotes.push(new Note(currentNote.pitchOrChord, targetNoteValue, currentNote.type));
        } else if (currentNoteIdx === (this.$_previousNoteIdx)) {
          let preTargetNoteValue = currentNote.value.add(leftNoteValueDifference);
          newNotes.push(new Note(currentNote.pitchOrChord, preTargetNoteValue, currentNote.type));
        } else if (currentNoteIdx === (this.$_nextNoteIdx)) {
          let postTargetNoteValue = currentNote.value.subtract(rightNoteValueDifference);
          newNotes.push(new Note(currentNote.pitchOrChord, postTargetNoteValue, currentNote.type));
        } else {
          newNotes.push(currentNote);
        }
      }
      let newNoteIdx = (insertRestNoteToFront)? this.selectedNoteIdx + 1 : this.selectedNoteIdx;
      let newPart = new PartInBar(newNotes, this.$_selectedPart.type);
      this.$emit(
        'update-part',
        this.$_selectedSectionIdx,
